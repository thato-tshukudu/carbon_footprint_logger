    ...
    <div className="min-h-screen bg-[var(--background)] flex items-center justify-center px-4 py-10">
      <div className="w-full max-w-md rounded-3xl border border-[var(--border)] bg-[var(--card)] p-8 shadow-[0_25px_60px_-25px_rgba(47,61,41,0.35)]">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-[var(--foreground)]">Join Carbon Tracker</h1>
          <p className="text-[var(--muted-text)]">Create your account to get started</p>
        </div>
        {error && (
          <div className="mb-4 rounded-2xl border border-[#d0ab6c] bg-[#fbf3e4] px-4 py-3 text-sm text-[#6d5126]">
            {error}
          </div>
        )}
        {success && (
          <div className="mb-4 rounded-2xl border border-dark-emerald-600 bg-dark-emerald-100 px-4 py-3 text-sm text-dark-emerald-900">
            {success}
          </div>
        )}
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-[var(--muted-text)] mb-2">Full Name</label>
            <div className="relative">
              <FaUser className="pointer-events-none absolute left-3 top-3 text-[var(--muted-text)]" />
              <input
                type="text"
                value={name}
        ...
        <div className="mt-6 text-center text-sm text-[var(--muted-text)]">
          <p>
            ...
            <Link href="/auth/signin" className="font-semibold text-dark-emerald-500 hover:text-dark-emerald-400">
              Sign in here
            </Link>
          </p>
        </div>
